    ...
    /* TODO: Distribute the material array at rank 0 to all other ranks */
        ...
        if(rank == 0){
            for(int i = 0; i < numprocesses; i++){
                int localX = 0;
                int localMaterialCount1 = 0;
                printf("rank 0 is creating local array for rank: %d\n", i);
                for(int x = i*(xSize/(numprocesses/2)); x < (i+1)*(xSize/(numprocesses/2)); x++){
                    int localY = 0;
                    for(int y = i*(ySize/(numprocesses/2)); y < (i+1)*(ySize/(numprocesses/2)); y++){
                        //if(i == 0){
                            local_material[lmi(localX, localY)] = material[mi(x, y)];
                            localX++;
                            localY++;
                            localMaterialCount1++;

                        //} else {
                        //    material_buffer[lmi(localX, localY)] = material[mi(x, y)];
                        //    localX++;
                        //    localY++;
                        //}
                    }
                }
                //send message to respective process
                if(!i){
                    //MPI_Send(material_buffer, localMaterialCount, MPI_FLOAT, i, 0, MPI_COMM_WORLD );
                    MPI_Send(local_material, localMaterialCount, MPI_FLOAT, i, 0, MPI_COMM_WORLD );
...
int main ( int argc, char **argv ){
    ...
    material_buffer = calloc( lsize, sizeof(float) );
